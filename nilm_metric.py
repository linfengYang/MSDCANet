...
def print_intermediate_results(target, prediction, threshold):
    ...
    print(f"Precision: {precision(tp, fp)}, Recall: {recall(tp, fn)}, F1: {f1(precision(tp, fp), recall(tp, fn))}")
...
def get_TP(target, prediction, threshold):
    ...
    target = 1 - np.clip(target, threshold, 0) / threshold   #大于阈值 threshold 的部分裁剪为 threshold
...
def get_FNR(target, prediction, threshold):
    '''
    compute the fnr rate
...
def get_abs_error(target, prediction):
    ...
    data1=(target - prediction)**2
    mean, std, min_v, max_v, quartile1, median, quartile2, rmse= get_statistics(data,data1)
    ecr=get_ecr(target, prediction)
    snr=get_snr(target,prediction)

    return mean, std, min_v, max_v, quartile1, median, quartile2, rmse,ecr,snr,data
...
def get_statistics(data,data1):

    mean = np.mean(data)
    rmse = np.sqrt(np.mean(data1))
    std = np.std(data)
    ...
    return mean, std, min_v, max_v, quartile1, median, quartile2,rmse
...
def recall_precision_accuracy_f1(pred, ground,threshold):
    ...
    if sum_samples == 0:
        ...
        return (res_recall,res_precision,res_accuracy,res_fnr,res_f1)
...
def get_ecr(target, prediction):
    '''
    compute the Energy Consumption Ratio (ECR)
    ...
    prediction: the prediction, np.array
    '''
    return np.sum(prediction) / np.sum(target)

def get_snr(target, prediction):
    '''
    ...
    noise_power = np.sum((target - prediction) ** 2)
